...
/**
 * Test Case: A014-BOOKADD_010
 * Type: Regression
 * Priority: High
 * Category: Negative
 * Feature: Book
 * Scenario: Empty Score - Validation Error
 *
 * Test Steps:
...
test.describe('A014-BOOKADD_010: Empty Score', () => {
  ...
  test(
    'should show error when score is empty',
    {
      tag: ['@add-book', '@regression', '@negative', '@A014-BOOKADD_010'],
    },
    async ({ authenticatedPage }) => {
      // Given - Add Book page is loaded
      await expect(authenticatedPage).toHaveURL(/.*add-book/);

      // When - Fill all fields except score and submit
